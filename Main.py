...
def insert_data_from_csv(csv_file_path):
    ...
    try:
        ...
        with open(csv_file_path, newline='', encoding='utf-8') as csvfile:
            reader = csv.DictReader(csvfile)  # Corrected class name
            for row in reader:
               ...
               # Process actors if present
        if 'Actors' in row:
                    Actors = row['Actors'].split('|') #Split actors by delimiter
                    for actor in Actors:
                        #Insert actor into actor_ref tabel if not already present
                        cur.execute(""" 
                            INSERT INTO actor_ref (actor_id, first_name, middle_name, last_name)
                            VALUES(%s, %s, %s, %s)
                            ON CONFLICT DO NOTHING
                        """, actor.split('', 1)) #Split actor into first_name, middle_name, 
...
def director_data_add(movies):
    ...
    for movie in movies:
        ...
        if directors:
            ...
            all_directors.extend (movies.directors)
    ...
    for director in all_directors:
        ...
        if count ==1:
            print ('1', dir_list)
            sql = 'INSERT INTO movies.director (first_name) VALUES (%s)'
            ...
            print('2', dir_list)
            sql = 'INSERT INTO movies_director (first_name, last_name) VALUES (%s)'
            first_name, last_name = dir_list[0]
            params = [first_name]
            doQuery(sql, params)

        elif count == 3:
            print('2', dir_list)
            sql = 'INSERT INTO movies_director (first_name, middle_name, last_name) VALUES (%s)'
